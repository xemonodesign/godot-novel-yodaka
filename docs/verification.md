...
## 2026-09-25 断片集の全17編をMAPに追加

- `make lint` 成功。`make test` はpytest 4件・カバレッジ98%台、Godot 4,233チェック成功。
- 断片集1（10編）・断片集2（7編）の全シートが夜に到達し、言葉のある7編（えらい・奇跡・幸せにする権利・かわいい・あす、あす・付き合ってあげる・宇宙の破滅）と、気づきの効果を持つ独白10編を区別して検証。全379ノードが到達可能。
- 週ごとの解放（第1週7か所、第2週+5、第3週+5）、各週3回・計12回の寄り道、3方針の通しプレイ（取得8〜13語、休息0〜2回）を検証。
- Chromium（Playwright、`build/web` をローカル配信）で通しプレイを完走：寄り道12回・夜16回・育てた言葉6語・取得8語、本編4章、カルテ2回、総括を経て結果画面へ。訪問先 ['cafe', 'hands', 'cover', 'park', 'parent', 'chime', 'unicorn', 'bridge', 'dream', 'karaoke', 'native', 'sushi']。最終ステータス [23, 86, 78, 77]。実行時エラー0件。
- 夜の「眠る」ボタンとMAPの「タイトル」ボタンが重なっていたため、MAP側を上に移動（連続クリックでタイトルへ飛ばない）。

![18か所のMAP](screenshots/map.png)
